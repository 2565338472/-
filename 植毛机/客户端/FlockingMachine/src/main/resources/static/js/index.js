			...
			if(d.code===200) {
				...
				if(data.length>0){
					...
					$(data).each(function(i, v) {
						...
						"<td>" + v.ng + "</td>" +//不良总数
						"<td id='total"+i+"'>" + v.total + "</td>" +//已完成量
						"<td id='outstandingAmount"+i+"'>" + v.outstandingAmount + "</td>" +//已完成量
...
function submitOrder(id,i) {
	...
	$.ajax({
		  type: 'POST',
		  url: "ProductionData/submit_order",
		  data: data,
...
function saveBaseInfo(i) {
	...
	$.ajax({
		  type: 'POST',
		  url: "ProductionData/saveBaseInfo",
		  data: data,
		  success: function(result) {
			  if(result.code == 200) {
				  layer.msg(result.message, {icon: 6});
	...
	$.get("/ProductionData/findByPageHistory",function(d){
		if(d.code === 200){
			...
			$(data).each(function(i, v) {
				...
				html += "<tr id='"+v.id+"'>" + 
				"<td>" + v.name + "</td>" +//机器名称
				"<td>"+v.workOrderNumber+"</td>" +//工单单号
				"<td>"+v.version+"</td>" +//产品单号
				...
				"<td>" + v.differenceHair + "</td>" +//差毛
				"<td id='total"+i+"'>" + v.total + "</td>" +//已完成量
				"<td id='outstandingAmount"+i+"'>" + v.outstandingAmount + "</td>" +//未完成量
				"<td>" + v.startTime + "</td>" +//开机时间
				"<td>" + v.endTime + "</td>";//关机时间
			});
